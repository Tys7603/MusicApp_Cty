...
const getListPlaylistByIdUser = async (userId) => {
  ...
  const query = "SELECT plu.playlist_user_id, plu.playlist_user_name, COUNT(s.song_id) AS song_count, s.song_image, GROUP_CONCAT(a.name_artist SEPARATOR ', ') AS name_artist " +
...
// playlisst user song by id user

const getListSongPlaylistByIdUser = async (playlistUserId) => {

  console.log(playlistUserId)

  const query = "SELECT s.song_id, s.song_name, s.song_image, s.song_url, a.name_artist " +
    "FROM playlist_user_song as pus " +
    "INNER JOIN Song as s ON s.song_id  = pus.song_id " +
    // "INNER JOIN Song_love as sl ON sl.song_id = s.song_id " +
    "INNER JOIN Album as a ON s.album_id = a.album_id " +
    "WHERE pus.playlist_user_id = ? " 
...
const getListPlaylistLoveByIdUser = async (userId) => {
  const query = `
    SELECT 
        ...
        pl.playlist_name, 
        pl.playlist_image, 
        GROUP_CONCAT(a.name_artist SEPARATOR ', ') AS name_artist, 
        plul.playlist_user_love_id 
    FROM 
        Playlist as pl 
    INNER JOIN 
        Playlist_user_love as plul ON pl.playlist_id = plul.playlist_id 
    INNER JOIN 
        Song as s ON pl.playlist_id = s.playlist_id 
    INNER JOIN 
        Album as a ON s.album_id = a.album_id 
    WHERE 
        plul.user_id = ?
    GROUP BY 
        pl.playlist_id, pl.playlist_name, pl.playlist_image, plul.playlist_user_love_id;
  `;
